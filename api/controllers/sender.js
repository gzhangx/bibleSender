...
function saveFunTypingRecord(req, res) {
    ...
    const { name, userName, wpm, wordCount } = req.body;
    if (!name) {
    ...
    return appendSheet('1fcSgz1vEh5I3NS5VXCx1BHitD_AAQrmUCXNJPPSyDYk', `'Sheet1'!A1`, [[new Date(), userName, name, wpm, wordCount]]).then(resok => {
        res.send({ ok: resok });
    }).catch(exc => {
        return res.send(exc);  
    })    
