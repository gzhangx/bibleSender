...
async function getAndSetAcccnAttendenceNumber(count, date) {
    ...
    const curVal = await readRanges(credentials.AcccnAttendenceSheetId, [`'${YYYY}'!A:A`, `'${YYYY}'!G:G`]).then(res=>{
  ...
  if (count > curVal.val) {
      ...
      await updateSheet(credentials.AcccnAttendenceSheetId, `'${YYYY}'!G${curVal.ind}:G${curVal.ind}`, [[count]]);
      return {
