...
function stdGetSaveToken(spreadsheetId,credentials, tokenMapper,token) {
  const path=['googleSheet', tokenMapper(spreadsheetId)];
  const existing=get(credentials,path);
  if(existing) return existing;
  if(!token) return null;
  set(credentials,path,token);
 ...
 */
function createSheetBase(ggcredentials, getToken, saveToken) {
  ...
  async function authorize(spreadsheetId) {
    ...
    let token=getToken(spreadsheetId);
...
module.exports={
  ...
  createSheet: () => {
    ...
    const mapper=() => 'ggtoken';
    const gc=credentials.googleSheet.installed
    return createSheetBase(gc,
      id => stdGetSaveToken(id,credentials,mapper),
      (id,token) => stdGetSaveToken(id,credentials,mapper,token),
    );
  } 
};