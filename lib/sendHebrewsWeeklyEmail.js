...
const hebrewsSheet = require('./getHebrewsSheet');
//const sheet = require('./getSheet').createSheet();
const spreadsheetId = '1uYTYzwjUN8tFpeejHtiGA5u_RtOoSBO8P1b2Qg-6Elk';
...
async function getTemplateData(curDate) {
  ...
  const getRows = async range => (await hebrewsSheet.readValues(range)).splice(1);
...
async function getSheetData(curDate = new Date()) {
  ...
  const getSheet = curDateD => hebrewsSeet.read(`'${getSheetName(curDateD)}'!A:P`);
...
function createMessage(templateAll, first, have) {
  ...
  const ownerLookup = (openHomeowner,name) => get(templateAll, `names[${openHomeOwner}.${name}`);
  ...
  const rpls = [data => map.reduce((acc, cur) => {
    ...
    ...['address', 'additionalInformation'].map(name => 
      data => data.replace(new RegExp(`[$]{${name}[(]([A-Z])[)]}`, 'gi'), (...m) => {
        ...
        const res = ownerLookup(owner, name);
        if (name === 'address' && !res) {
  ...
  const doMonth = async month => {
    ...
    const getChurchSchedule = async () => {
      ...
      const mapped = await Promise.map(moment.monthsShort().slice(month, month+3), async (mon, monthNum) => {
        const rv = await hebrewsSheet.readValues('13x-0a9jKCFxSVmN90K9QOpamLUNWb4V5_cvE8RaeYDw', `'${mon}'!B5:F30`);
    ...
    await hebrewsSheet.updateSheet(spreadsheetId, `'${getSheetName(curDateD)}'!A3:E${3 + data.length}`, data);
    ...
    await hebrewsSheet.append(`'${getSheetName(curDateD)}'!A:E`, churchdata);
  }
...
function createMessage2021(templateAll, first, have) {  
  const getRowDataByLetter = x => getRowData(addrToPos(x[1] || x[0]));
...
async function checkSheetNotice(curDateD = new Date(), sendEmail = true) {
  ...
  const scheduleData = await hebrewsSheet.readValues(`'Schedule'!${valuesRange[0]}:${valuesRange[1]}`).then(r => {    
    return r.map(d => {
      const dateStr = d[0];
      if (!dateStr) return;
      ...
      if ((ret[addrToPos('L')] || '').trim().toLowerCase() !== 'yes') return null;
      return ret;
    }).filter(x => x);
  });
