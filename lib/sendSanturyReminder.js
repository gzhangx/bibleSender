const spreadsheetId = '1OJFvWYJtireTblpUuUpWz1q10EQThjKT4ggp1mv8yXE';
...
async function checkSanturyNotice(curDateD = new Date(), sendEmail = true) {    
    ...
    const message = {
      ...
      const emails = [rows[3],rows[5],rows[7],rows[9]].filter(x=>x).join(',');
      ...
      招待員須知：
              ...
              7.    請組長在每月初提醒組員不要忘記。
      `;
      console.log(`${emails} ${emails.subject}`);
      //console.log(message);
      if (sendEmail)
