...
function getNextSunday() {
  return moment().isoWeekday(7).toDate();
}
...
async function checkSanturyNotice(curDateD = new Date(), sendEmail = true) {        
    ...
    let nmSdi = (curDateD.getDate() < 15)? getNextSunday() : getSdayNextMonth(curDateD);
    const year = nmSdi.getFullYear();
  const month = nmSdi.getMonth() + 1;
  const ops = await sheet.getSheetOps(spreadsheetId);
    const res = await ops.readSheet(`'${year}'!A:J`);
